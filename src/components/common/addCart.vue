    ...
    <transition name="fade">
        <div class="add-cart" v-if="cartStatus">
            ...
            <div class="cart-content">
                <div class="detail"><img :src="cartDetail.img[0]" />
                    <div class="info">
                        <div class="name">{{cartDetail.name}}</div>
                        <div class="price">¥{{cartDetail.price}}</div>
                        <div class="add"><span @click="reduceCartNum(cartDetail.id)" :class="cartNum===0?'none':''">
                                <icon name="minus"></icon>
                            </span><span class="num">{{cartNum}}</span><span @click="addCartNum(cartDetail.id)">
                                <icon name="plus"></icon>
                            </span></div>
                    </div>
                </div>
                <div class="handle">
                    <div class="total">合计:<span class="price">¥{{cartDetail.price*cartNum}}</span></div>
                    <div class="order" @click="toShopCar(cartDetail)">立即购买</div>
                </div>
            </div>
        </div>
    </transition>
