<template>
  <div class="shopCar">
    <div class="shopCar-main">
        <p class="shopCar-title" v-if="canChosenList.length&gt;0">购物车</p>
        <p class="shopCar-remind" v-if="canChosenList.length===0"> <img :src="emptyImg" /></p>
        <ul class="goods-list">
            <li v-for="(item,index) in canChosenList" :key="index" v-if="item.num&gt;0"><span class="choose-icon" @click.stop="changeChooseStatus(item,index)">
                    <icon :name="item.chooseStatus?'check-circle-o':'circle-o'" scale="1.4"></icon>
                </span><img :src="item.img[0]" />
                <div class="name">{{item.name}}</div>
                <div class="price">
                    <div class="single">单价:¥{{item.price}}</div>
                    <div class="all">合计:¥{{item.price*item.num}}</div>
                </div>
                <div class="add"><span @click.stop="reduceCartNum(item,index)" :class="item.num&lt;=1?'none':''">
                        <icon name="minus" scale=".8"></icon>
                    </span><span class="num">{{item.num}}</span><span @click.stop="addCartNum(item,index)">
    ...
    <div class="total"><span class="choose-icon" @click.stop="changeChooseAll">
            <icon :name="chooseAllStatus&amp;&amp;canChosenList.length&gt;0?'check-circle-o':'circle-o'" scale="1.4"></icon>
        </span>
        <div class="content">合计 <span class="price">¥ {{totalNum}}</span><span class="order" @click.stop="confirmOrder">结算</span></div>
    </div>
</div>
